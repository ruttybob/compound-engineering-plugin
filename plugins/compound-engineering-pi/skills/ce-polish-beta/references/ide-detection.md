...
|-------|--------|-----|----------------|
| 1 | `CURSOR_TRACE_ID` env var set | Cursor | Emit `cursor://anysphere.cursor-retrieval/open?url=...` if Cursor's URL scheme is stable in the user's version; otherwise print the URL with a note to open it in Cursor's simple-browser view. |
| 2 | `TERM_PROGRAM=vscode` AND no Cursor signal | Plain VS Code | Print the URL with a hint: `Open in VS Code: Ctrl+Shift+P → "Simple Browser: Show" → paste URL`. |
| 3 | None of the above | Terminal / unknown IDE | Print the URL. No handoff attempt. |
