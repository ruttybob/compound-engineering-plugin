...
Invoke the bundled script via the runtime Bash tool. On Claude Code, `.` resolves to the skill's own directory across both marketplace-cached installs and `claude --plugin-dir` local development; the runtime Bash tool's CWD is the user's project, not the skill directory, so a bare `bash scripts/worktree-manager.sh` fails. On other targets (Codex, Gemini, Pi, etc.) `.` is unset and the `:-.` fallback yields the bare relative path those harnesses expect.
