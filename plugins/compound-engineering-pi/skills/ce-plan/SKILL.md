...
- subagent with agent="ce-repo-research-analyst" and task="Scope: technology, architecture, patterns. {planning context summary}"
- subagent with agent="ce-learnings-researcher" and task="planning context summary"
Collect:
...
- **Implementation-guidance** — run in parallel:
  - subagent with agent="ce-best-practices-researcher" and task="planning context summary"
  - subagent with agent="ce-framework-docs-researcher" and task="planning context summary, with exact frameworks/versions from Phase 1.1 where available"
- **Landscape / option-discovery** — subagent with agent="ce-web-researcher" and task="focus hint, planning context summary". When the request targets projects on a code host (e.g., "competitors on GitHub"), name the discovery dimensions in the focus hint: project names and URLs, release recency and activity, CLI/UX shape, install path, docs and examples, plugin/extension surfaces, recurring issue themes, and license — treating star counts as a weak signal only.
...
- subagent with agent="ce-spec-flow-analyzer" and task="planning context summary, research findings"
